/*
  ...
  Compile like this:
  gcc -g -Wall -Og -o elf2vri elf2vri.c -lm
...
long int inSize = 0; // files can be large, this needs to be long int
long int outSize = 0;
long int rawInSize = 0;
long int rawOutSize = 0;
uint32_t currentAddress = 0; // must be unsigned I think
uint32_t autoAddress = 1; // must be unsigned I think
...
unsigned int PurgeSections(struct Section *s, FILE *fp, unsigned int *d, unsigned int addr, int literalRun, int rleRun,
                           int verbose) {
    if (verbose) {
...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-h")) {
            ...
            rawSect[0][rawSectors[0]].addr = autoAddress ? (uint32_t) -1 : addr;
            ...
            rawSect[1][rawSectors[1]].addr = autoAddress ? (uint32_t) -1 : addr;
    ...
    if (oName && !!strcmp(oName, "-")) {
        ...
        if (!ofp) {
            ...
            goto finally; // Cleanup
        }
    } else {
        ofp = stdout;
        verbose = 0; // Default to stdout, disable verbose
    }
    ...
    for (i = 0; i < rawSectors[0]; i++) {
        ...
        if (verbose) {
            printf("RAW compressed %ld bytes to %ld "
                   "(%.1f%% compression)\n",
    ...
    if (verbose) {
        printf("RLE/BSS/strip compressed %ld bytes to %ld "
               "(%.1f%% compression)\n",
